...
// (the same as for short inting point  multiplication and addition),
...
__global__ void ndviGPU(
	...
	if(xIndex < vectorN && yIndex < elementN)
	{
		unsigned int index = yIndex * (vectorN) + xIndex;
		
		d_C[index] = (float)(d_A[index] - d_B[index]) / (float)(d_A[index] + d_B[index]);
	}
}
